'use strict';

goog.provide('Blockly.Msg.Webduino.en');

goog.require('Blockly.Msg.Webduino');

// Sample

Blockly.Msg.SAMPLE_COMFIRM_MSG = "\"Load Sample\" will load sample code to replace your workspace, or you can click \"+\" to add sample code into your workspace.";

// BlockSvg ContextMenu

Blockly.Msg.MAINURL = "https://tutorials.webduino.io/zh-tw/docs/";
Blockly.Msg.UTMURL = '?utm_source=cloud-blockly&utm_medium=contextMenu&utm_campaign=tutorials';

Blockly.Msg.HELP = "Tutorials";
Blockly.Msg.TOOL = "Tool";

Blockly.Msg.VARIABLES_SET_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/variables.html" + Blockly.Msg.UTMURL;
Blockly.Msg.VARIABLES_GET_HELPURL = Blockly.Msg.VARIABLES_SET_HELPURL;

Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL = "https://tutorials.webduino.io/zh-tw/docs/blockly/standard/functions.html" + Blockly.Msg.UTMURL;
Blockly.Msg.PROCEDURES_DEFRETURN_HELPURL = Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL;
Blockly.Msg.PROCEDURES_CALLNORETURN_HELPURL = Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL;
Blockly.Msg.PROCEDURES_CALLRETURN_HELPURL = Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL;
Blockly.Msg.PROCEDURES_IFRETURN_HELPURL = Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL;

Blockly.Msg.TEXT_TEXT_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/text.html" + Blockly.Msg.UTMURL;
Blockly.Msg.TEXT_JOIN_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_APPEND_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_INDEXOF_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_CHARAT_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_GET_SUBSTRING_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_CHANGECASE_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_TRIM_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_PROMPT_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_LENGTH_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_ISEMPTY_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;
Blockly.Msg.TEXT_PRINT_HELPURL = Blockly.Msg.TEXT_TEXT_HELPURL;

Blockly.Msg.MATH_NUMBER_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/math.html" + Blockly.Msg.UTMURL;
Blockly.Msg.MATH_ARITHMETIC_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_SINGLE_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_TRIG_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_CONSTANT_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_CHANGE_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_ROUND_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_ONLIST_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_MODULO_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_CONSTRAIN_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_RANDOM_INT_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;
Blockly.Msg.MATH_RANDOM_FLOAT_HELPURL = Blockly.Msg.MATH_NUMBER_HELPURL;

Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/lists.html" + Blockly.Msg.UTMURL;
Blockly.Msg.LISTS_CREATE_WITH_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_REPEAT_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_LENGTH_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_ISEMPTY_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_INDEX_OF_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_GET_INDEX_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_SET_INDEX_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_GET_SUBLIST_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;
Blockly.Msg.LISTS_SPLIT_HELPURL = Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL;

Blockly.Msg.CONTROLS_IF_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/logic.html" + Blockly.Msg.UTMURL;
Blockly.Msg.LOGIC_COMPARE_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;
Blockly.Msg.LOGIC_OPERATION_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;
Blockly.Msg.LOGIC_NEGATE_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;
Blockly.Msg.LOGIC_BOOLEAN_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;
Blockly.Msg.LOGIC_NULL_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;
Blockly.Msg.LOGIC_TERNARY_HELPURL = Blockly.Msg.CONTROLS_IF_HELPURL;

Blockly.Msg.CONTROLS_REPEAT_HELPURL = Blockly.Msg.MAINURL + "blockly/standard/loops.html" + Blockly.Msg.UTMURL;
Blockly.Msg.CONTROLS_WHILEUNTIL_HELPURL = Blockly.Msg.CONTROLS_REPEAT_HELPURL;
Blockly.Msg.CONTROLS_FOR_HELPURL = Blockly.Msg.CONTROLS_REPEAT_HELPURL;
Blockly.Msg.CONTROLS_FOREACH_HELPURL = Blockly.Msg.CONTROLS_REPEAT_HELPURL;
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_HELPURL = Blockly.Msg.CONTROLS_REPEAT_HELPURL;

Blockly.Msg.COLOUR_PICKER_HELPURL = Blockly.Msg.MAINURL + "blockly/index.html" + Blockly.Msg.UTMURL;
Blockly.Msg.COLOUR_RANDOM_HELPURL = Blockly.Msg.COLOUR_PICKER_HELPURL;
Blockly.Msg.COLOUR_RGB_HELPURL= Blockly.Msg.COLOUR_PICKER_HELPURL;
Blockly.Msg.COLOUR_BLEND_HELPURL= Blockly.Msg.COLOUR_PICKER_HELPURL;


// Main

Blockly.Msg.VARIABLES_SET = "Set %1  %2";
Blockly.Msg.LISTS_CREATE_WITH_INPUT_WITH = "Create List";
Blockly.Msg.TEXT_JOIN_TITLE_CREATEWITH = "Create String";

// Math

Blockly.Msg.WEBDUINO_MATH_ROUND = "Round";
Blockly.Msg.WEBDUINO_MATH_ROUND_UP = "Round Up";
Blockly.Msg.WEBDUINO_MATH_ROUND_DOWN = "Round Down";
Blockly.Msg.WEBDUINO_MATH_ROUND_TO = "to the Decimal Point";
Blockly.Msg.WEBDUINO_MATH_ROUND_NUM = "";
Blockly.Msg.WEBDUINO_MATH_SCALE = "Scale Conversion, Source";
Blockly.Msg.WEBDUINO_MATH_SCALE_OMIN = "(origin) Min";
Blockly.Msg.WEBDUINO_MATH_SCALE_OMAX = "(origin) Max";
Blockly.Msg.WEBDUINO_MATH_SCALE_CMIN = "(conversion) Min";
Blockly.Msg.WEBDUINO_MATH_SCALE_CMAX = "(conversion) Max";
Blockly.Msg.NUMBER_CONVERT = "Convert to";
Blockly.Msg.NUMBER_BINARY = "Binary";
Blockly.Msg.NUMBER_OCTAL = "Octal";
Blockly.Msg.NUMBER_DECTIMAL = "Dectimal";
Blockly.Msg.NUMBER_HEX = "Hexadecimal";
Blockly.Msg.NUMBER_STRING = "String";

// Demo

Blockly.Msg.DEMO_SHOW = "Show:";
Blockly.Msg.DEMO_TEXT_SIZE = "Text Size";
Blockly.Msg.DEMO_TEXT_LINEHEIGHT = "Line height";
Blockly.Msg.DEMO_TEXT_COLOR = "Text Color";
Blockly.Msg.DEMO_LIGHT_CLICK = "Click Bulb, Do";
Blockly.Msg.DEMO_LIGHT_STATE = "Set Bulb's State:";
Blockly.Msg.DEMO_LIGHT_IFELSE = "Bulb's Status is:";
Blockly.Msg.DEMO_LIGHT_TOGGLE = "Light Toggle";
Blockly.Msg.DEMO_BUTTON_CLICK = "Click";
Blockly.Msg.DEMO_BUTTON_BTN1 = "Button1";
Blockly.Msg.DEMO_BUTTON_BTN2 = "Button2";
Blockly.Msg.DEMO_BUTTON_BTN3 = "Button3";
Blockly.Msg.DEMO_BUTTON_BTN4 = "Button4";
Blockly.Msg.DEMO_BUTTON_BTN5 = "Button5";
Blockly.Msg.DEMO_BUTTON_CLICK_DO = "Do";
Blockly.Msg.DEMO_IMAGE_URL = "Image URL:";
Blockly.Msg.DEMO_IMAGE_WIDTH = "Image Width";
Blockly.Msg.DEMO_IMAGE_HEIGHT = "Height";
Blockly.Msg.DEMO_IMAGE_ROTATE = "Image Rotate:";
Blockly.Msg.DEMO_IMAGE_OPACITY = "Image Transparency:";
Blockly.Msg.DEMO_IMAGE_POS = "Image ";
Blockly.Msg.DEMO_IMAGE_X = "Horizontal";
Blockly.Msg.DEMO_IMAGE_Y = "Vertical";
Blockly.Msg.DEMO_IMAGE_MOVE = "Move:";
Blockly.Msg.DEMO_IMAGE_PIXEL = "Pixels";
Blockly.Msg.DEMO_AREA_COLOR = "Set Area's Color:";
Blockly.Msg.DEMO_AREA_COLOR_INPUT = "Choose Color, Do";
Blockly.Msg.DEMO_AREA_COLOR_CHOOSE = "Color:";
Blockly.Msg.DEMO_RANGE_MIN = "Set Range, Min:";
Blockly.Msg.DEMO_RANGE_MAX = "Max:";
Blockly.Msg.DEMO_RANGE_STEP = "Interval:";
Blockly.Msg.DEMO_RANGE_DEFAULT = "Default:";
Blockly.Msg.DEMO_RANGE_INPUT = "Slider Range, Do";
Blockly.Msg.DEMO_RANGE_SHOW = "Show Value:";
Blockly.Msg.DEMO_RANGE_VALUE = "Value:";
Blockly.Msg.DEMO_YOUTUBE_CHANGEID = "Change";
Blockly.Msg.DEMO_YOUTUBE_CHANGEID_ID = "Replace ID:";
Blockly.Msg.DEMO_SERVER_URL = "Video source url:";
Blockly.Msg.DEMO_TRACKING_SET = "Set Tracking";
Blockly.Msg.DEMO_TRACKING = "Track";
Blockly.Msg.DEMO_TRACKING_FACE = "Face";
Blockly.Msg.DEMO_TRACKING_COLOR = "Color";
Blockly.Msg.DEMO_TRACKING_DO = "When tracking succeeds, Do";
Blockly.Msg.DEMO_TRACKING_STANDBY = "When tracking standby, Do";
Blockly.Msg.DEMO_TRACKING_RUN = "Start";
Blockly.Msg.DEMO_TRACKING_STOP = "Stop";
Blockly.Msg.DEMO_TRACKING_TRACK = "Tracking";
Blockly.Msg.DEMO_TRACKING_FACESETTING = "Face Tracking Setting (optional)";
Blockly.Msg.DEMO_TRACKING_VAL = "Tracking Value";
Blockly.Msg.DEMO_TRACKING_VALTOTAL = "Total ( face )";
Blockly.Msg.DEMO_TRACKING_VALAGE = "Age ( age )";
Blockly.Msg.DEMO_TRACKING_VALEMOTION = "emotion ( emotion )";
Blockly.Msg.DEMO_TRACKING_VALCOLOR = "color ( color )";
Blockly.Msg.DEMO_CONTROLLER_BTN = "Remote's Button";
Blockly.Msg.DEMO_CONTROLLER_BTNCLICK = "Click";
Blockly.Msg.DEMO_CONTROLLER_BTNDOWN = "Down";
Blockly.Msg.DEMO_CONTROLLER_BTNUP = "Up";
Blockly.Msg.DEMO_CONTROLLER_BTNRED = "Red";
Blockly.Msg.DEMO_CONTROLLER_BTNGREEN = "Green";
Blockly.Msg.DEMO_CONTROLLER_BTNBLUE = "Blue";
Blockly.Msg.DEMO_CONTROLLER_BTNYELLOW = "Yellow";
Blockly.Msg.DEMO_CONTROLLER_BTNDO = "Do";
Blockly.Msg.DEMO_CONTROLLER_SCREEN = "Remote's screen, show";
Blockly.Msg.DEMO_CONTROLLER_RANGEMIN = "Remote's range, min";
Blockly.Msg.DEMO_CONTROLLER_RANGEMAX = "Max";
Blockly.Msg.DEMO_CONTROLLER_RANGESTEP = "Interval";
Blockly.Msg.DEMO_CONTROLLER_RANGEDEFAULT = "Default";
Blockly.Msg.DEMO_CONTROLLER_RANGEDO = "Do";
Blockly.Msg.DEMO_CONTROLLER_RANGE_VALUE = "Value of Remote's Range";
Blockly.Msg.DEMO_CONTROLLER_SCREENSHOW = "On Remote's Screen,";
Blockly.Msg.DEMO_CONTROLLER_SCREENBG = "background";
Blockly.Msg.DEMO_CONTROLLER_SCREENTEXT = "Text";
Blockly.Msg.DEMO_CONTROLLER_SCREENCOLOR = "'s color'";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION = "Mobile Orientation";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_DO = "Do";
Blockly.Msg.DEMO_CONTROLLER_MOBILE = "Orientation";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_ALPHA = "Alpha";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_BETA = "Beta";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_GAMMA = "Gamma";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_VAL1 = "'s Value, Round:'";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_VAL2 = "";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_REMOVE = "Stop Orientation";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_ACCELERATION_X = "x";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_ACCELERATION_Y = "y";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_ACCELERATION_Z = "z";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_ACCELERATION_VAL2 = Blockly.Msg.DEMO_CONTROLLER_MOBILE_DEVICEORIENTATION_VAL2;
Blockly.Msg.DEMO_CONTROLLER_MOBILE_ACCELERATION = "Acceleration ";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_TYPE1 = "Orientation";
Blockly.Msg.DEMO_CONTROLLER_MOBILE_TYPE2 = "Motion";

Blockly.Msg.DEMO_CONTROLLER_CHART = "Chart";
Blockly.Msg.PIE_TITLE = "Pie Chart Title";
Blockly.Msg.PIE_SUB_TITLE = "Subtitle";
Blockly.Msg.DEMO_PIE_DATA = "List";
Blockly.Msg.PIE_REFRESH = "Refresh pie chart";
Blockly.Msg.PIE_VALUE = "Value";
Blockly.Msg.PIE_DESCRIPTION = "Description";
Blockly.Msg.PIE_AUTO_REFRESH = "Auto refresh pie chart every";
Blockly.Msg.PIE_AUTO_REFRESH_SEC = "second";

// DataTransfer

Blockly.Msg.DATA_TRANSFER_SET = "Set";
Blockly.Msg.DATA_TRANSFER_USE = "Use";
Blockly.Msg.DATA_TRANSFER_FORMAT = "Send Data, Data Format:";
Blockly.Msg.DATA_TRANSFER_SEND_STRING = "Send String";
Blockly.Msg.DATA_TRANSFER_RTN_STRING = "Callback";
Blockly.Msg.DATA_TRANSFER_RECV_STRING = "Received string";

// Console

Blockly.Msg.CUSTOM_JS_CONSOLE = "Console:";
Blockly.Msg.CUSTOM_JS_Date = "Date:";
Blockly.Msg.CUSTOM_JS_Date_1 = "year / month / date";
Blockly.Msg.CUSTOM_JS_Date_2 = "month / date / year";
Blockly.Msg.CUSTOM_JS_Date_3 = "date / month / year";
Blockly.Msg.CUSTOM_JS_Date_4 = "year";
Blockly.Msg.CUSTOM_JS_Date_5 = "month";
Blockly.Msg.CUSTOM_JS_Date_6 = "date";
Blockly.Msg.CUSTOM_JS_Time = "Time:";
Blockly.Msg.CUSTOM_JS_Time_1 = "hours : minutes : seconds";
Blockly.Msg.CUSTOM_JS_Time_2 = "hours";
Blockly.Msg.CUSTOM_JS_Time_3 = "minutes";
Blockly.Msg.CUSTOM_JS_Time_4 = "seconds";

// Control

Blockly.Msg.WEBDUINO_TEMP_SET = "Set temp";
Blockly.Msg.WEBDUINO_TEMP_SET_NAME = "   Name:";
Blockly.Msg.WEBDUINO_TEMP_SET_VALUE = "Value:";
Blockly.Msg.WEBDUINO_TEMP_GET = "Get temp";
Blockly.Msg.WEBDUINO_TEMP_GET_NAME = "name:";
Blockly.Msg.WEBDUINO_TEMP_GET_VALUE = "'s value";
Blockly.Msg.WEBDUINO_RECONGNITION = "Start Recognition ( Chrome only, not support iOS )";
Blockly.Msg.WEBDUINO_RECONGNITION_INTER = "   interimResults:";
Blockly.Msg.WEBDUINO_RECONGNITION_MOBILE = "(Mobile Checked: Off)";
Blockly.Msg.WEBDUINO_RECONGNITION_LANG = "Language:";
Blockly.Msg.WEBDUINO_RECONGNITION_IF = "If Result Contains:";
Blockly.Msg.WEBDUINO_RECONGNITION_DO = "Do";
Blockly.Msg.WEBDUINO_RECONGNITION_CONTENT = "Recognized Text";
Blockly.Msg.WEBDUINO_RECONGNITION_STOP = "Stop";
Blockly.Msg.WEBDUINO_RECONGNITION_START = "Start";
Blockly.Msg.WEBDUINO_RECONGNITION_TEXT = "Recognition";
Blockly.Msg.WEBDUINO_SPEECH_MALE = "male";
Blockly.Msg.WEBDUINO_SPEECH_FEMALE = "female";
Blockly.Msg.WEBDUINO_SPEECH = "  speech:";
Blockly.Msg.WEBDUINO_SPEECH_APPID = "AppID:";
Blockly.Msg.WEBDUINO_SPEECH_SET = "Language:";
Blockly.Msg.WEBDUINO_SPEECH_SEX = "   Gender:";
Blockly.Msg.WEBDUINO_SPEAK_TEXT = "Speak";
Blockly.Msg.WEBDUINO_SPEAK_SETTING = "Setting";
Blockly.Msg.WEBDUINO_SPEAK_WHEN = "When";
Blockly.Msg.WEBDUINO_SPEAK_END = "Ended";
Blockly.Msg.WEBDUINO_SPEAK_START = "Started";
Blockly.Msg.WEBDUINO_SPEAK_DO = "Do";
Blockly.Msg.WEBDUINO_SPEAK_RESUME = "Resume";
Blockly.Msg.WEBDUINO_SPEAK_PAUSE = "Pause";
Blockly.Msg.WEBDUINO_SPEAK_CANCEL = "Cancel";
Blockly.Msg.WEBDUINO_SPEAK_READ = "Speak";
Blockly.Msg.WEBDUINO_SPEAK_LANG = "Language";
Blockly.Msg.WEBDUINO_SPEAK_TW = "zh-TW";
Blockly.Msg.WEBDUINO_SPEAK_US = "en-US";
Blockly.Msg.WEBDUINO_SPEAK_JP = "ja-JP";
Blockly.Msg.WEBDUINO_SPEAK_VOLUME = " Volume";
Blockly.Msg.WEBDUINO_SPEAK_PITCH = " Pitch";
Blockly.Msg.WEBDUINO_SPEAK_P20 = "2";
Blockly.Msg.WEBDUINO_SPEAK_P15 = "1.5";
Blockly.Msg.WEBDUINO_SPEAK_P10 = "1";
Blockly.Msg.WEBDUINO_SPEAK_P05 = "0.5";
Blockly.Msg.WEBDUINO_SPEAK_P01 = "0.1";
Blockly.Msg.WEBDUINO_SPEAK_RATE = " Rate";
Blockly.Msg.WEBDUINO_SPEAK_R20 = "2";
Blockly.Msg.WEBDUINO_SPEAK_R15 = "1.5";
Blockly.Msg.WEBDUINO_SPEAK_R10 = "1";
Blockly.Msg.WEBDUINO_SPEAK_R07 = "0.7";
Blockly.Msg.WEBDUINO_SPEAK_R05 = "0.5";

// Repeat

Blockly.Msg.WEBDUINO_STATUS_REPEAT = "Status Change, repeat:";
Blockly.Msg.WEBDUINO_STATUS_NUM = "";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_STATUSNUM = "Status ";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_DELAY = " for";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_SECS = "seconds:";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_ADD = "Add Status";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_STATUS = "Status";
Blockly.Msg.WEBDUINO_STATUS_FOREVER = "Status repeat switch";
Blockly.Msg.WEBDUINO_STATUS_FOREVER_NAME = ", name:";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_STOP = "Stop";
Blockly.Msg.WEBDUINO_STATUS_REPEAT_STOPCONTENT = "";

// Text logic

Blockly.Msg.TEXTLOGIC_INSIDE = "";
Blockly.Msg.TEXTLOGIC_HAS = "Contains";
Blockly.Msg.TEXTLOGIC_NO = "Does not contain";
Blockly.Msg.TEXTLOGIC_TEXT = "words";

// Keyboard

Blockly.Msg.WEBDUINO_KEYBOARD = "Monitor Keyboard Event, When";
Blockly.Msg.WEBDUINO_KEYBOARD_KEYDOWN = "Key down";
Blockly.Msg.WEBDUINO_KEYBOARD_KEYUP = "Key up";
Blockly.Msg.WEBDUINO_KEYBOARD_STOP = "Stop Monitoring";
Blockly.Msg.WEBDUINO_KEYBOARD_KEY = "Key";
Blockly.Msg.WEBDUINO_KEYBOARD_DO = ", Do:";
Blockly.Msg.WEBDUINO_KEYBOARD_SPACE = "Space";
Blockly.Msg.WEBDUINO_KEYBOARD_UP = "Up";
Blockly.Msg.WEBDUINO_KEYBOARD_DOWN = "Down";
Blockly.Msg.WEBDUINO_KEYBOARD_LEFT = "Left";
Blockly.Msg.WEBDUINO_KEYBOARD_RIGHT = "Right";
Blockly.Msg.WEBDUINO_KEYBOARD_KONAMI = "When Konami Code Entered, Do:";

// Delay

Blockly.Msg.WEBDUINO_TIMER_AFTER = "(Async) After";
Blockly.Msg.WEBDUINO_TIMER_SECOND = "Second(s)";
Blockly.Msg.WEBDUINO_TIMER_DO = "Do";
Blockly.Msg.WEBDUINO_INTERVAL_EVERY = "(Async) Every";
Blockly.Msg.WEBDUINO_INTERVAL_SECOND = "Second(s)";
Blockly.Msg.WEBDUINO_INTERVAL_DO = "Do";
Blockly.Msg.WEBDUINO_DELAY = "Delay";
Blockly.Msg.WEBDUINO_DELAY_SECONDS = "Second(s)";

// Board

Blockly.Msg.WEBDUINO_BOARD = "Board";
Blockly.Msg.WEBDUINO_BOARD_SERVER = "Private Server, IP";
Blockly.Msg.WEBDUINO_BOARD_WIFI = "Wi-Fi";
Blockly.Msg.WEBDUINO_BOARD_SERIAL = "Serial Port";
Blockly.Msg.WEBDUINO_BOARD_BLUETOOTH = "Bluetooth";
Blockly.Msg.WEBDUINO_BOARD_WEBSOCKET = "WebSocket";
Blockly.Msg.WEBDUINO_BOARD_SIMULATOR = "Simulator";
Blockly.Msg.WEBDUINO_BOARD_MULTI = "  Multi";
Blockly.Msg.WEBDUINO_BOARD_CHAIN = "Series";
Blockly.Msg.WEBDUINO_BOARD_CHAIN_OK = "When in a Series Circuit";
Blockly.Msg.WEBDUINO_BOARD_CHAIN_DO = "Do";
Blockly.Msg.WEBDUINO_QUERY_PIN = "Query Pin:";
Blockly.Msg.WEBDUINO_QUERY_PIN_NUM = "'s state";
Blockly.Msg.WEBDUINO_PIN_DO = "Do";
Blockly.Msg.WEBDUINO_PIN_STATE = "Pin state";
Blockly.Msg.WEBDUINO_BOARD_IF_ERROR = "When Board Goes Offline";
Blockly.Msg.WEBDUINO_BOARD_ERROR_DO = "Do";
Blockly.Msg.WEBDUINO_BOARD_SAMPLING = "  Sampling";
Blockly.Msg.WEBDUINO_USE = "Use";
Blockly.Msg.WEBDUINO_CONNECT_TO = "Connect to";
Blockly.Msg.WEBDUINO_SMART = "Smart";
Blockly.Msg.WEBDUINO_MARK = "MARK 1";
Blockly.Msg.WEBDUINO_FLY = "Fly";

// LED

Blockly.Msg.WEBDUINO_LED = "LED";
Blockly.Msg.WEBDUINO_PIN = "Pin:";
Blockly.Msg.WEBDUINO_LED_PIN = "Pin:";
Blockly.Msg.WEBDUINO_LED_STATE = "";
Blockly.Msg.WEBDUINO_LED_TOGGLE = "Toggle State";
Blockly.Msg.WEBDUINO_LED_INTENSITY = "Intensity (0-1.0):";
Blockly.Msg.WEBDUINO_LED_SET = "Turn";
Blockly.Msg.WEBDUINO_LED_STATE_IS = "'s State:";
Blockly.Msg.WEBDUINO_LED_STATE_DO = "Do";
Blockly.Msg.WEBDUINO_LED_BLINK = "Blink Every";
Blockly.Msg.WEBDUINO_LED_BLINK_SEC = "Second(s)";
Blockly.Msg.WEBDUINO_LED_BLINK_DO = "Do";

// Relay

Blockly.Msg.WEBDUINO_RELAY = "Relay, Pin:";
Blockly.Msg.WEBDUINO_RELAY_SET_STATE = "Set State";

// RGB

Blockly.Msg.WEBDUINO_RGBLED = "RGB LED";
Blockly.Msg.WEBDUINO_RGBLED_CATHODE = "RGB LED (Cathode)";
Blockly.Msg.WEBDUINO_RGBLED_RED = "Red";
Blockly.Msg.WEBDUINO_RGBLED_GREEN = "Green";
Blockly.Msg.WEBDUINO_RGBLED_BLUE = "Blue";
Blockly.Msg.WEBDUINO_RGBLED_SETCOLOR = "Set Color";

// Smart Servo

Blockly.Msg.WEBDUINO_SMART_SERVO_NEW = "Smart Servo , ID:";
Blockly.Msg.WEBDUINO_SMART_SERVO_SET = "Smart Servo";
Blockly.Msg.WEBDUINO_SMART_SERVO_SET_POS = " , Position";
Blockly.Msg.WEBDUINO_SMART_SERVO_SET_TIME = " , time(10ms)";
Blockly.Msg.WEBDUINO_SMART_SERVO_SET_SPEED = " , speed";

// Button

Blockly.Msg.WEBDUINO_BUTTON_NEW = "Button, Pin";
Blockly.Msg.WEBDUINO_BUTTON_PULLUP_NEW = "Pull-up Button Switch, Pin:";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_WHEN = "When";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_WAS = "is";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_TO = "";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_DO = "Do";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_PRESSED = "Pressed";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_RELEASED = "Released";
Blockly.Msg.WEBDUINO_BUTTON_EVENT_LONGPRESS = "Long Pressed";

// Shock

Blockly.Msg.WEBDUINO_SHOCK_NEW = "Shock switch, Pin:";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_WHEN = "When";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_WAS = "is";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_TO = "";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_DO = "Do";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_HIGH = "Short Circuit";
Blockly.Msg.WEBDUINO_SHOCK_EVENT_LOW = "Open Circuit";

// Joystick
Blockly.Msg.WEBDUINO_JOYSTICK = 'Joystick';
Blockly.Msg.WEBDUINO_JOYSTICK_VALUE = 'Detected Value of';
Blockly.Msg.WEBDUINO_JOYSTICK_DO = 'Do';

// PIR

Blockly.Msg.WEBDUINO_PIR = "PIR Motion Sensor, Pin:";
Blockly.Msg.WEBDUINO_PIR_WHEN = "When";
Blockly.Msg.WEBDUINO_PIR_STATUS_DETECTED = "Detected";
Blockly.Msg.WEBDUINO_PIR_STATUS_ENDED = "not Detected";
Blockly.Msg.WEBDUINO_PIR_DETECTED = "PIR's Change";
Blockly.Msg.WEBDUINO_PIR_DO = "Do";

// Buzzer

Blockly.Msg.WEBDUINO_BUZZER = "Buzzer, Pin:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1 = "Add Music, name:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1_EDIT = "Notes and Tempos:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1_TONE = "Note:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1_PITCH = "   Pitch:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1_TEMPOS = "   Tempo:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC1_NO = "None";
Blockly.Msg.WEBDUINO_BUZZER_USE = "Let";
Blockly.Msg.WEBDUINO_BUZZER_PLAY = "play:";
Blockly.Msg.WEBDUINO_BUZZER_LET = "Let";
Blockly.Msg.WEBDUINO_BUZZER_EVENT_STOP = "Stop";
Blockly.Msg.WEBDUINO_BUZZER_EVENT_PAUSE = "Pause";
Blockly.Msg.WEBDUINO_BUZZER_EVENT_RESUMEPLAY = "Resume";
Blockly.Msg.WEBDUINO_BUZZER_EVENT_PLAY = "Playback";
Blockly.Msg.WEBDUINO_BUZZER_CHECK = "Check if";
Blockly.Msg.WEBDUINO_BUZZER_STATE = "'s State is";
Blockly.Msg.WEBDUINO_BUZZER_STATE_STOP = "Stopped";
Blockly.Msg.WEBDUINO_BUZZER_STATE_PAUSE = "Paused";
Blockly.Msg.WEBDUINO_BUZZER_STATE_PLAYING = "Playing";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC2 = "Add Music, Name:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC2_NOTES = "Notes:";
Blockly.Msg.WEBDUINO_BUZZER_MUSIC2_TEMPOS = "Tempo:";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC = "Choose Music:";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC1 = "Super Mario";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC2 = "DoReMi";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC3 = "Big Brother and Father";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC4 = "Super Mario Chord";
Blockly.Msg.WEBDUINO_BUZZER_CHOOSE_MUSIC5 = "Merry Christmas";
Blockly.Msg.WEBDUINO_BUZZER_SINGLE = "Use";
Blockly.Msg.WEBDUINO_BUZZER_SINGLE_TONE = "Play Single Tone, Note";
Blockly.Msg.WEBDUINO_BUZZER_SINGLE_PITCH = "Pitch";
Blockly.Msg.WEBDUINO_BUZZER_SINGLE_TEMPOS = "Tempo";

// Max7219

Blockly.Msg.WEBDUINO_MATRIX = "LED Matrix (Max7219) ";
Blockly.Msg.WEBDUINO_MATRIX_DIN = "DIN:";
Blockly.Msg.WEBDUINO_MATRIX_CS = "CS:";
Blockly.Msg.WEBDUINO_MATRIX_CLK = "CLK:";
Blockly.Msg.WEBDUINO_MATRIX_CODE = "Show Image from Code:";
Blockly.Msg.WEBDUINO_MATRIX_TIMES = "Time (ms):";
Blockly.Msg.WEBDUINO_MATRIX_DUR = "Duration (ms):";
Blockly.Msg.WEBDUINO_MATRIX_LIST = "Code (list):";
Blockly.Msg.WEBDUINO_MATRIX_STOP = "";
Blockly.Msg.WEBDUINO_MATRIX_ANIMATE = "Stop Animation";
Blockly.Msg.WEBDUINO_MATRIX_CLOSE = "Close";
Blockly.Msg.WEBDUINO_MATRIX_HORSE = "Marquee  Direction";
Blockly.Msg.WEBDUINO_MATRIX_LEFT = "Left";
Blockly.Msg.WEBDUINO_MATRIX_RIGHT = "Right";
Blockly.Msg.WEBDUINO_MATRIX_SPEED = " Speed (frame/ms):";
Blockly.Msg.WEBDUINO_MATRIX_HORSEODE = "Code:";
Blockly.Msg.WEBDUINO_MATRIX_ALPHABET = "Alphabet";
Blockly.Msg.WEBDUINO_MATRIX_ALPHABET2 = "Alphabet (A~Z, a~z)";
Blockly.Msg.WEBDUINO_MATRIX_UPPERCASE = "Uppercase";
Blockly.Msg.WEBDUINO_MATRIX_LOWERCASE = "Lowercase";
Blockly.Msg.WEBDUINO_MATRIX_NUM = "Number";
Blockly.Msg.WEBDUINO_MATRIX_NUM2 = "Number (0~99)";
Blockly.Msg.WEBDUINO_MATRIX_IMG = "Image";

// SSD1306

Blockly.Msg.WEBDUINO_SSD1306 = "OLED ( SSD1306 ), SCL ";
Blockly.Msg.WEBDUINO_SSD1306_SDA = "  SDA";
Blockly.Msg.WEBDUINO_SSD1306_PRINT = "OLED Show:";
Blockly.Msg.WEBDUINO_SSD1306_IMAGE = "OLED Image:";
Blockly.Msg.WEBDUINO_SSD1306_CLEAR = "Clear OLED ";
Blockly.Msg.WEBDUINO_SSD1306_SET = "Set";
Blockly.Msg.WEBDUINO_SSD1306_LOCATE_X = "X Cursor:";
Blockly.Msg.WEBDUINO_SSD1306_LOCATE_Y = "Y Cursor:";
Blockly.Msg.WEBDUINO_SSD1306_TEXT_SIZE = "OLED Set Text Size (1~7):";

// Soil

Blockly.Msg.WEBDUINO_SOIL = "Soil, Analog Pin:";
Blockly.Msg.WEBDUINO_SOIL_DETECTED = "Detecting";
Blockly.Msg.WEBDUINO_SOIL_DO = "Do";
Blockly.Msg.WEBDUINO_SOIL_VAL = "Detected Value";
Blockly.Msg.WEBDUINO_SOIL_STOP = "Stop Detecting";

// ADXL345

Blockly.Msg.WEBDUINO_ADXL345 = "ADXL345, SDA";
Blockly.Msg.WEBDUINO_ADXL345_SCL = "  SCL";
Blockly.Msg.WEBDUINO_ADXL345_DO = "do:";
Blockly.Msg.WEBDUINO_ADXL345_S = "'s";
Blockly.Msg.WEBDUINO_ADXL345_VAL = "Value";
Blockly.Msg.WEBDUINO_START_DETECTING = "Start Detecting";
Blockly.Msg.WEBDUINO_STOP_DETECTING = "Stop Detecting";

// HX711

Blockly.Msg.WEBDUINO_HX711 = "HX711, SCK";
Blockly.Msg.WEBDUINO_HX711_DT = "  DT";
Blockly.Msg.WEBDUINO_HX711_ON = "Start Detecting";
Blockly.Msg.WEBDUINO_HX711_DO = "Do";
Blockly.Msg.WEBDUINO_HX711_VAL = "Value";
Blockly.Msg.WEBDUINO_HX711_OFF = "Stop Detecting";

// Barcode

Blockly.Msg.WEBDUINO_BARCODE = "Barcode Scanner, Rx";
Blockly.Msg.WEBDUINO_BARCODE_TX = "  Tx";
Blockly.Msg.WEBDUINO_BARCODE_ON = "Start Detecting";
Blockly.Msg.WEBDUINO_BARCODE_DO = "Do";
Blockly.Msg.WEBDUINO_BARCODE_VAL = "Barcode";
Blockly.Msg.WEBDUINO_BARCODE_OFF = "Stop Detecting";

// Firebase

Blockly.Msg.WEBDUINO_FIREBASE_NAME = "< Firebase > name:";
Blockly.Msg.WEBDUINO_FIREBASE_ADDCOLUMN = 'Add Column';
Blockly.Msg.WEBDUINO_FIREBASE_COLUMN = 'column';
Blockly.Msg.WEBDUINO_FIREBASE_WRITEDATA = "Add data";
Blockly.Msg.WEBDUINO_FIREBASE_WRITEDATACOLUMN = 'column ';
Blockly.Msg.WEBDUINO_FIREBASE_WRITEDATANAME = ' name:';
Blockly.Msg.WEBDUINO_FIREBASE_WRITEDATAVAL = '  value:';
Blockly.Msg.WEBDUINO_FIREBASE_URL = ",url:";
Blockly.Msg.WEBDUINO_FIREBASE_DO = "do:";
Blockly.Msg.WEBDUINO_FIREBASE_WRITE = "Write";
Blockly.Msg.WEBDUINO_FIREBASE_IS = "為";
Blockly.Msg.WEBDUINO_FIREBASE_READ = "If";
Blockly.Msg.WEBDUINO_FIREBASE_ATTR = "changes, read property";
Blockly.Msg.WEBDUINO_FIREBASE_S = "is";
Blockly.Msg.WEBDUINO_FIREBASE_TO = "to";
Blockly.Msg.WEBDUINO_FIREBASE_CLEAR = "Clear Database";
Blockly.Msg.WEBDUINO_FIREBASE_READONCE = "Read";
Blockly.Msg.WEBDUINO_FIREBASE_ATTRONCE = "'s property";
Blockly.Msg.WEBDUINO_FIREBASE_DATAALL = "All";
Blockly.Msg.WEBDUINO_FIREBASE_DATALAST = "Last one";
Blockly.Msg.WEBDUINO_FIREBASE_NAME_OF_DATA_1 = "Name of data 1";
Blockly.Msg.WEBDUINO_FIREBASE_NAME_OF_DATA_2 = "Name of data 2";
Blockly.Msg.WEBDUINO_FIREBASE_VALUE = "Value";
Blockly.Msg.WEBDUINO_FIREBASE_NAME_TEXT = "Name";
Blockly.Msg.WEBDUINO_FIREBASE_OBJECT = "Object";
Blockly.Msg.WEBDUINO_FIREBASE_ADD_OBJECT = "Add Object";
Blockly.Msg.WEBDUINO_FIREBASE_CREATE_OBJECT = "Create Object";

// RFID

Blockly.Msg.WEBDUINO_RFID_ON = "Starts Detecting";
Blockly.Msg.WEBDUINO_RFID_UID = "Detected UID";
Blockly.Msg.WEBDUINO_RFID_UIDIS = "Detected UID is";
Blockly.Msg.WEBDUINO_RFID_IF = "If";
Blockly.Msg.WEBDUINO_RFID_DO = "Do";
Blockly.Msg.WEBDUINO_RFID_OUT = "Removed";
Blockly.Msg.WEBDUINO_RFID_OFF = "Stops Detecting";

/*
.                   .                       o8o            oooo
.o8                 .o8                       `"'            `888
.o888oo oooo  oooo  .o888oo  .ooooo.  oooo d8b oooo   .oooo.    888
888   `888  `888    888   d88' `88b `888""8P `888  `P  )88b   888
888    888   888    888   888   888  888      888   .oP"888   888
888 .  888   888    888 . 888   888  888      888  d8(  888   888
"888"  `V88V"V8P'   "888" `Y8bod8P' d888b    o888o `Y888""8o o888o
*/

// Tutorials - LED

Blockly.Msg.WEBDUINO_LIGHT = "Bulb";
Blockly.Msg.WEBDUINO_LIGHT_CLICK = "Click bulb, do";
Blockly.Msg.WEBDUINO_LIGHT_STATUS = "Bulb's status  ";

Blockly.Msg.WEBDUINO_TWO_LED_LIGHT_BULB = "Bulb";
Blockly.Msg.WEBDUINO_TWO_LED_LIGHT_BULB_LEFT = "Left";
Blockly.Msg.WEBDUINO_TWO_LED_LIGHT_BULB_RIGHT = "Right";
Blockly.Msg.WEBDUINO_TWO_LED_LIGHT_BULB_CLICK = "Click bulb, do";
Blockly.Msg.WEBDUINO_TWO_LED_LIGHT_BULB_STATUS = "Bulb's status  ";

Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_SET = "Set Blinking Clock:";
Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_TIME = "time:";
Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_MSEC = "ms ( 1/1000 sec )";
Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_STATUS1 = "status 1:";
Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_STATUS2 = "status 2:";

Blockly.Msg.WEBDUINO_STOP_CLOCK = "Stop Clock:";

// Tutorials - RGB

Blockly.Msg.WEBDUINO_RGB_LED_AREA_COLOR = "Set demo area color:";
Blockly.Msg.WEBDUINO_RGB_LED_BTN_CLICK = "Click";
Blockly.Msg.WEBDUINO_RGB_LED_BTN_BUTTON = "Button";
Blockly.Msg.WEBDUINO_RGB_LED_BTN_DO = "do:";

Blockly.Msg.WEBDUINO_RGB_LED_RANGE_CHANGE = "Range change";
Blockly.Msg.WEBDUINO_RGB_LED_RANGE_DO = "do:";
Blockly.Msg.WEBDUINO_RGB_LED_CURRENT_COLOR = "Current color";

Blockly.Msg.WEBDUINO_RGB_LED_START_DANCING_CLOCK_NAME = "Set Color Clock:";
Blockly.Msg.WEBDUINO_RGB_LED_START_DANCING_TIME = "Time:";
Blockly.Msg.WEBDUINO_RGB_LED_START_DANCING_MSEC = Blockly.Msg.WEBDUINO_TWO_LED_START_BLANKING_MSEC;
Blockly.Msg.WEBDUINO_RGB_LED_START_DANCING_CHANGE = "Change:";

Blockly.Msg.WEBDUINO_RGB_LED_START_DANCING_STATUS = " Status:";

// Tutorials - UltraSonic

Blockly.Msg.WEBDUINO_ULTRASONIC_SET_SHOW_DISTANCE = "Show Distance:";

Blockly.Msg.WEBDUINO_ULTRASONIC_IMAGE_SIZE = "Image Size:";
Blockly.Msg.WEBDUINO_ULTRASONIC_IMAGE_URL = "Image URL:";

Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_VOLUME = "Music Volume";
Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_PLAY = "Music:";
Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_PLAY_PLAY = "Paly";
Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_PLAY_PAUSE = "Pause";
Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_PLAY_STOP = "Stop";
Blockly.Msg.WEBDUINO_ULTRASONIC_MUSIC_ADD = "Add Music";

Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE = "Set Youtube: ";
Blockly.Msg.DEMO_YOUTUBE_DEFAULT = "Default";
Blockly.Msg.DEMO_YOUTUBE_DEFAULT_1 = "Video";
Blockly.Msg.DEMO_YOUTUBE_DEFAULT_2 = "Video List";
Blockly.Msg.DEMO_YOUTUBE_DEFAULT_ID = "'s ID is";
Blockly.Msg.DEMO_YOUTUBE_PLAY = "Play";
Blockly.Msg.DEMO_YOUTUBE_LIST = "Video List";
Blockly.Msg.DEMO_YOUTUBE_LIST_PRE = "Previous Video";
Blockly.Msg.DEMO_YOUTUBE_LIST_NEXT = "Next Video";
Blockly.Msg.DEMO_YOUTUBE_LIST_LISTNUM = "Video List's";
Blockly.Msg.DEMO_YOUTUBE_LIST_NUM = "Video";
Blockly.Msg.DEMO_YOUTUBE_VIDEO_ID = " Default Video ID: ";
Blockly.Msg.DEMO_YOUTUBE_CHANGEID_ID = "Replace ID: ";
Blockly.Msg.DEMO_YOUTUBE_CURRENTTIME = "Current Time (sec)";
Blockly.Msg.DEMO_YOUTUBE_SET = "Set";
Blockly.Msg.DEMO_YOUTUBE_SEEKTO = "Seek to";
Blockly.Msg.DEMO_YOUTUBE_SEC = "sec";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SET = "Set";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_VOLUME = "Volume:";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SPEED = "Speed:";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_VERYSLOW = "Very Slow";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SLOW = "Slow";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_NORMAL = "Normal";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_FAST = "Fast";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_VERYFAST = "Very Fast";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SUPERFAST = "Super Fast";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUS = " Status:";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SETPLAY = "Play";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SETPAUSE = "Pause";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_SETSTOP = "Stop";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUSPLAY = "Playing";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUSPAUSE = "Paused";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUSCUE = "Stopped";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUSSTOP = "Ended";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_STATUSSTART = "Ready";
Blockly.Msg.WEBDUINO_ULTRASONIC_YOUTUBE_DO = "Do";

// Tutorials - Button

Blockly.Msg.WEBDUINO_SHOW_TEXT = "Show text: ";

Blockly.Msg.WEBDUINO_SHOW_CALCULATE_NUMBER = "Show Number  Current number";
Blockly.Msg.WEBDUINO_SHOW_CALCULATE_PLUS = "+";
Blockly.Msg.WEBDUINO_SHOW_CALCULATE_MINUS = "-";
Blockly.Msg.WEBDUINO_SHOW_CALCULATE_TIMES = "*";
Blockly.Msg.WEBDUINO_SHOW_CALCULATE_DIVIDED = "/";

Blockly.Msg.WEBDUINO_SHOW_SET_CURRENT_NUMBER = "Set Current number";

Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE = "Image move";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_MOVE = "";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_PX = "Pixel";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_RESET = "Reset image position";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_UP = "up";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_DOWN = "down";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_LEFT = "left";
Blockly.Msg.WEBDUINO_BUTTON_CHANGE_IMAGE_RIGHT = "right";

Blockly.Msg.WEBDUINO_BUTTON_GAME_ADD_SINGLEGAME = "Add <Single> Game";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC = "NPC:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL = "  Level:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_DISTANCE = "  Distance:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_USER = "User:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_USER_BUTTON = "  Button:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_USER_EVENT = "Button event:";
Blockly.Msg.WEBDUINO_BUTTON_GAME_USER_RUN_FORWARD = "Character runs forward";
Blockly.Msg.WEBDUINO_BUTTON_GAME_USER_RUN_PIXEL = "pixel";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_CAT = "Cat";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LION = "Lion";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_SQU = "Squirrel";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_PICA = "Mario";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_MAN1 = "Peter";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_MAN2 = "RYU";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL_5 = "Very hard";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL_4 = "Hard";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL_3 = "Normal";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL_2 = "Easy";
Blockly.Msg.WEBDUINO_BUTTON_GAME_NPC_LEVEL_1 = "Very easy";

// Tutorials - Servo

Blockly.Msg.WEBDUINO_SERVO_CLICKBTN = "Click button:";
Blockly.Msg.WEBDUINO_SERVO_BTNLEFT90 = "Left 90°";
Blockly.Msg.WEBDUINO_SERVO_BTNLEFT60 = "Left 60°";
Blockly.Msg.WEBDUINO_SERVO_BTNLEFT30 = "Left 30°";
Blockly.Msg.WEBDUINO_SERVO_BTNCENTER = "Center";
Blockly.Msg.WEBDUINO_SERVO_BTNRIGHT30 = "Right 30°";
Blockly.Msg.WEBDUINO_SERVO_BTNRIGHT60 = "Right 60°";
Blockly.Msg.WEBDUINO_SERVO_BTNRIGHT90 = "Right 90°";
Blockly.Msg.WEBDUINO_SERVO_DO = "Do";
Blockly.Msg.WEBDUINO_SERVO_SET_ANGLE = "Set Angle (1°-180°)";
Blockly.Msg.WEBDUINO_SERVO_CURRENT_ANGLE = "Current Angle";
Blockly.Msg.WEBDUINO_SERVO_PLUS = "+";
Blockly.Msg.WEBDUINO_SERVO_MINUS = "-";

// Tutorials - Buzzer

Blockly.Msg.WEBDUINO_BUZZER_SHOW = "Show";
Blockly.Msg.WEBDUINO_BUZZER_SHOW_NOTESANDTEMPOS = "'s notes and tempos";
Blockly.Msg.WEBDUINO_BUZZER_CLICK = "Click";
Blockly.Msg.WEBDUINO_BUZZER_DO = "do:";

// Tutorials - Photcell

Blockly.Msg.WEBDUINO_PHOTOCELL_SHOW = "Show Value:";

// Tutorials - Max7219

Blockly.Msg.WEBDUINO_MAX7219_CLICKBTN = "Click";
Blockly.Msg.WEBDUINO_MAX7219_BTN1 = "Button 1";
Blockly.Msg.WEBDUINO_MAX7219_BTN2 = "Button 2";
Blockly.Msg.WEBDUINO_MAX7219_BTN3 = "Button 3";
Blockly.Msg.WEBDUINO_MAX7219_BTNSTOP = "Stop";
Blockly.Msg.WEBDUINO_MAX7219_BTNOFF = "Turn off";
Blockly.Msg.WEBDUINO_MAX7219_DO = "do:";

// Tutorials - ADXL345

Blockly.Msg.WEBDUINO_ADXL_SHOW = "Show";
Blockly.Msg.WEBDUINO_ADXL_SHOW_X = "x";
Blockly.Msg.WEBDUINO_ADXL_SHOW_Y = "y";
Blockly.Msg.WEBDUINO_ADXL_SHOW_Z = "z";
Blockly.Msg.WEBDUINO_ADXL_SHOW_R = "roll";
Blockly.Msg.WEBDUINO_ADXL_SHOW_P = "pitch";
Blockly.Msg.WEBDUINO_ADXL_SHOW_IS = "is";
Blockly.Msg.WEBDUINO_ADXL_ANGLE = "Image rotate ";

// Tutorials - irrecv

Blockly.Msg.WEBDUINO_IRRRECV_SHOW = "Show code:";
Blockly.Msg.WEBDUINO_IRRRECV_COLOR = "Area color:";

// Tutorials - RFID

Blockly.Msg.WEBDUINO_RFID_SHOW = "Show RFID code";

// Google Sheets

Blockly.Msg.WEBDUINO_GOOGLESHEETS = "Load Google Sheets";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_URL = "URL";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_SHEETNAME = "Sheet";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_ADDCOLUMN = "Add Column";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_COLUMN = "Column";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_SHEETS = "Sheet";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_WRITEDATA = "Write data into";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_COLUMN_A = "Column A:";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_COLUMN_B = "Column B:";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_COLUMNWITE = "Column";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_COLUMNVALUE = ":";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_READ_FROM = "Read data from"
Blockly.Msg.WEBDUINO_GOOGLESHEETS_READROW = " Column";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_READCOL = " Row";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_READDO = "Do";
Blockly.Msg.WEBDUINO_GOOGLESHEETS_READDATA = "Sheet data";




/*
 Yb        dP 888888 88""Yb 88""Yb 88 888888
 Yb  db  dP  88__   88__dP 88__dP 88   88
  YbdPYbdP   88""   88""Yb 88""Yb 88   88
   YP  YP    888888 88oodP 88oodP 88   88
*/

// Web:Bit - Tutorial url

Blockly.Msg.TUTORIAL_URL = "https://tutorials.webduino.io/zh-tw/docs/webbit/";

// Web:Bit - Variable

Blockly.Msg.BIT_VARIABLES = "item";
Blockly.Msg.BIT_VARIABLES_CHANGE = "change";
Blockly.Msg.BIT_VARIABLES_CHANGE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/variables.html#change";
Blockly.Msg.VARIABLES_SET_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/variables.html";
Blockly.Msg.VARIABLES_GET_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/variables.html#use";

// Web:Bit - Logic

Blockly.Msg.TEXTLOGIC_INSIDE = "";
Blockly.Msg.TEXTLOGIC_HAS = "has";
Blockly.Msg.TEXTLOGIC_NO = "doesn't has";
Blockly.Msg.TEXTLOGIC_TEXT = "text";
Blockly.Msg.BIT_LOGIC_INDEXOF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic05";
Blockly.Msg.BIT_LOGIC_IS_EMPTY = "is empty";
Blockly.Msg.BIT_LOGIC_IS_EMPTY_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic04";
Blockly.Msg.BIT_LOGIC_IS_EVEN = "is";
Blockly.Msg.BIT_LOGIC_IS_EVEN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic03";
Blockly.Msg.CONTROLS_IF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#main";
Blockly.Msg.LOGIC_COMPARE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic01";
Blockly.Msg.LOGIC_OPERATION_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic02";
Blockly.Msg.LOGIC_NEGATE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic06";
Blockly.Msg.LOGIC_BOOLEAN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic07";
Blockly.Msg.LOGIC_NULL_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic08";
Blockly.Msg.LOGIC_TERNARY_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/logic.html#logic09";
Blockly.Msg.BIT_LOGIC_IS_EVEN_EVEN = "even";
Blockly.Msg.BIT_LOGIC_IS_EVEN_ODD = "odd";
Blockly.Msg.BIT_LOGIC_IS_EVEN_INTEGER = "integer";
Blockly.Msg.BIT_LOGIC_IS_EVEN_FLOAT = "float";
Blockly.Msg.BIT_LOGIC_IS_EVEN_STRING = "string";
Blockly.Msg.BIT_LOGIC_IS_EVEN_ARRAY = "array";

// Web:Bit - Loop

Blockly.Msg.WEBDUINO_DELAY = "Wait";
Blockly.Msg.WEBDUINO_DELAY_SECONDS = "seconds";
Blockly.Msg.WEBDUINO_DELAY_SECONDS_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop01";
Blockly.Msg.BIT_LOOP_FOREVER = "Repeat forever, sync:";
Blockly.Msg.BIT_LOOP_FOREVER_DO = "Do";
Blockly.Msg.BIT_LOOP_FOREVER_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop04";
Blockly.Msg.BIT_LOOP_FOREVER_WHILEDO = "Repeat forever, sync:";
Blockly.Msg.BIT_LOOP_FOREVER_WHILEDO_IF = "If";
Blockly.Msg.BIT_LOOP_FOREVER_WHILEDO_DO = "Do";
Blockly.Msg.BIT_LOOP_FOREVER_WHILEDO_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop05";
Blockly.Msg.BIT_LOOP_STOP = "Stop";
Blockly.Msg.BIT_LOOP_STOP_ALL = "all";
Blockly.Msg.BIT_LOOP_STOP_THIS = "this";
Blockly.Msg.BIT_LOOP_STOP_REPEAT = "repeat";
Blockly.Msg.BIT_LOOP_STOP_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop08";
Blockly.Msg.BIT_LOOP_EXT_REPEAT = "Repeat";
Blockly.Msg.BIT_LOOP_EXT = "times, sync:";
Blockly.Msg.BIT_LOOP_EXT_DO = "Do";
Blockly.Msg.BIT_LOOP_EXT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop02";
Blockly.Msg.BIT_LOOP_FOR = "Coun with";
Blockly.Msg.BIT_LOOP_FOR_FROM = "from";
Blockly.Msg.BIT_LOOP_FOR_TO = "to";
Blockly.Msg.BIT_LOOP_FOR_NUM = "by";
Blockly.Msg.BIT_LOOP_FOR_ASYNC = "sync:";
Blockly.Msg.BIT_LOOP_FOR_DO = "Do";
Blockly.Msg.BIT_LOOP_FOR_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop03";
Blockly.Msg.BIT_LOOP_FOREACH = "For each item";
Blockly.Msg.BIT_LOOP_FOREACH_ARRAY = "in list";
Blockly.Msg.BIT_LOOP_FOREACH_ASYNC = "sync";
Blockly.Msg.BIT_LOOP_FOREACH_DO = "Do";
Blockly.Msg.BIT_LOOP_FOREACH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/loop.html#loop06";

// Web:Bit - Math

Blockly.Msg.MATH_NUMBER_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math01";
Blockly.Msg.MATH_ARITHMETIC_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math04";
Blockly.Msg.MATH_SINGLE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math10";
Blockly.Msg.MATH_TRIG_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math11";
Blockly.Msg.MATH_CONSTANT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math12";
Blockly.Msg.MATH_CHANGE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html";
Blockly.Msg.MATH_ROUND_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html";
Blockly.Msg.MATH_ONLIST_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math09";
Blockly.Msg.MATH_MODULO_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html";
Blockly.Msg.MATH_CONSTRAIN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math06";
Blockly.Msg.MATH_RANDOM_INT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math02";
Blockly.Msg.MATH_RANDOM_FLOAT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math03";
Blockly.Msg.BIT_MATH_REMAINDER = "Remainder of";
Blockly.Msg.BIT_MATH_REMAINDER_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math05";
Blockly.Msg.WEBDUINO_MATH_ROUND = "round";
Blockly.Msg.WEBDUINO_MATH_ROUND_UP = "round up";
Blockly.Msg.WEBDUINO_MATH_ROUND_DOWN = "round down";
Blockly.Msg.WEBDUINO_MATH_ROUND_TO = "to";
Blockly.Msg.WEBDUINO_MATH_ROUND_NUM = "decimal place";
Blockly.Msg.BIT_MATH_ROUND_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math07";
Blockly.Msg.WEBDUINO_MATH_SCALE = "Scaling, source:";
Blockly.Msg.WEBDUINO_MATH_SCALE_OMIN = "(origin) min";
Blockly.Msg.WEBDUINO_MATH_SCALE_OMAX = "(origin) max";
Blockly.Msg.WEBDUINO_MATH_SCALE_CMIN = "(after) min";
Blockly.Msg.WEBDUINO_MATH_SCALE_CMAX = "(after) max";
Blockly.Msg.BIT_MATH_SCALE = Blockly.Msg.TUTORIAL_URL + "basic/math.html#math08";

// Web:Bit - Text

Blockly.Msg.TEXT_TEXT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text01";
Blockly.Msg.TEXT_JOIN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text12";
Blockly.Msg.TEXT_APPEND_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text03";
Blockly.Msg.TEXT_INDEXOF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.TEXT_CHARAT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.TEXT_GET_SUBSTRING_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.TEXT_CHANGECASE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text08";
Blockly.Msg.TEXT_TRIM_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text09";
Blockly.Msg.TEXT_PROMPT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.TEXT_LENGTH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text11";
Blockly.Msg.TEXT_ISEMPTY_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.TEXT_PRINT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html";
Blockly.Msg.BIT_TEXT_WRAP = "Newline";
Blockly.Msg.BIT_TEXT_WRAP_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text02";
Blockly.Msg.BIT_TEXT_APPEND = "To";
Blockly.Msg.BIT_TEXT_APPEND_TO = "append text";
Blockly.Msg.BIT_TEXT_APPEND_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text03";
Blockly.Msg.BIT_TEXT_REPLACE = "In";
Blockly.Msg.BIT_TEXT_REPLACE_WHICH = "find";
Blockly.Msg.BIT_TEXT_REPLACE_FIRST = "first";
Blockly.Msg.BIT_TEXT_REPLACE_ALL = "all";
Blockly.Msg.BIT_TEXT_REPLACE_TO = "replace of text";
Blockly.Msg.BIT_TEXT_REPLACE_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text04";
Blockly.Msg.BIT_TEXT_INDEXOF = "In";
Blockly.Msg.BIT_TEXT_INDEXOF_FIND = "find";
Blockly.Msg.BIT_TEXT_INDEXOF_FIRST = "first";
Blockly.Msg.BIT_TEXT_INDEXOF_LAST = "last";
Blockly.Msg.BIT_TEXT_INDEXOF_STRING = "occurrence of text";
Blockly.Msg.BIT_TEXT_INDEXOF_POSITION = "";
Blockly.Msg.BIT_TEXT_INDEXOF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text05";
Blockly.Msg.BIT_TEXT_CHARAT = "In text";
Blockly.Msg.BIT_TEXT_CHARAT_NUM1 = "";
Blockly.Msg.BIT_TEXT_CHARAT_NUM2 = "";
Blockly.Msg.BIT_TEXT_CHARAT_NUM = "letter #";
Blockly.Msg.BIT_TEXT_CHARAT_ENDNUM = "letter # from end";
Blockly.Msg.BIT_TEXT_CHARAT_FIRST = "first letter";
Blockly.Msg.BIT_TEXT_CHARAT_LAST = "last letter";
Blockly.Msg.BIT_TEXT_CHARAT_RANDOM = "random letter";
Blockly.Msg.BIT_TEXT_CHARAT_S = "get";
Blockly.Msg.BIT_TEXT_CHARAT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text06";
Blockly.Msg.BIT_TEXT_GETSTR = "In text";
Blockly.Msg.BIT_TEXT_GETSTR_FROM1 = "to";
Blockly.Msg.BIT_TEXT_GETSTR_FROM2 = "to";
Blockly.Msg.BIT_TEXT_GETSTR_TO1 = "";
Blockly.Msg.BIT_TEXT_GETSTR_TO2 = "";
Blockly.Msg.BIT_TEXT_GETSTR_S = "get sybstring from";
Blockly.Msg.BIT_TEXT_GETSTR_NUM = "letter #";
Blockly.Msg.BIT_TEXT_GETSTR_ENDNUM = "letter # from end";
Blockly.Msg.BIT_TEXT_GETSTR_FIRST = "first letter";
Blockly.Msg.BIT_TEXT_GETSTR_LAST = "last letter";
Blockly.Msg.BIT_TEXT_GETSTR_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text07";
Blockly.Msg.NUMBER_CONVERT = "Translate to";
Blockly.Msg.NUMBER_BINARY = "Binary";
Blockly.Msg.NUMBER_OCTAL = "Octal";
Blockly.Msg.NUMBER_DECTIMAL = "Decimal";
Blockly.Msg.NUMBER_HEX = "Hexadecimal";
Blockly.Msg.NUMBER_STRING = "from";
Blockly.Msg.NUMBER_CONVERT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text10";
Blockly.Msg.BIT_TEXT_LENGTH_STRING = "Length of";
Blockly.Msg.BIT_TEXT_LENGTH = "";
Blockly.Msg.BIT_TEXT_LENGTH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/text.html#text11";
Blockly.Msg.BIT_TEXT_TO_NUM = "To";
Blockly.Msg.BIT_TEXT_TO_NUM_NUM = "number";
Blockly.Msg.BIT_TEXT_TO_NUM_STRING = "text";

// Web:Bit - Array

Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array01";
Blockly.Msg.LISTS_CREATE_WITH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array02";
Blockly.Msg.LISTS_REPEAT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array03";
Blockly.Msg.LISTS_LENGTH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_ISEMPTY_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_INDEX_OF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_GET_INDEX_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_SET_INDEX_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_GET_SUBLIST_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.LISTS_SPLIT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html";
Blockly.Msg.BIT_ARRAY_SET = "In list";
Blockly.Msg.BIT_ARRAY_SET_NUM = "#";
Blockly.Msg.BIT_ARRAY_SET_ENDNUM = "# from end";
Blockly.Msg.BIT_ARRAY_SET_FIRST = "first";
Blockly.Msg.BIT_ARRAY_SET_LAST = "last";
Blockly.Msg.BIT_ARRAY_SET_RANDOM = "random";
Blockly.Msg.BIT_ARRAY_SET_NUMIS1 = "as";
Blockly.Msg.BIT_ARRAY_SET_NUM1 = "";
Blockly.Msg.BIT_ARRAY_SET_NUMIS2 = "as";
Blockly.Msg.BIT_ARRAY_SET_NUM2 = "";
Blockly.Msg.BIT_ARRAY_SET_SET = "set";
Blockly.Msg.BIT_ARRAY_SET_INSERT = "insert at";
Blockly.Msg.BIT_ARRAY_SET_REMOVE = "remove";
Blockly.Msg.BIT_ARRAY_SET_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array04";
Blockly.Msg.BIT_ARRAY_GET = "In list";
Blockly.Msg.BIT_ARRAY_GET_NUM1 = "";
Blockly.Msg.BIT_ARRAY_GET_NUM2 = "";
Blockly.Msg.BIT_ARRAY_GET_NUM = "#";
Blockly.Msg.BIT_ARRAY_GET_ENDNUM = "# from end";
Blockly.Msg.BIT_ARRAY_GET_FIRST = "first";
Blockly.Msg.BIT_ARRAY_GET_LAST = "last";
Blockly.Msg.BIT_ARRAY_GET_RANDOM = "random";
Blockly.Msg.BIT_ARRAY_GET_GET = "get";
Blockly.Msg.BIT_ARRAY_GET_GETANDREMOVE = "get and remove";
Blockly.Msg.BIT_ARRAY_GET_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array05";
Blockly.Msg.BIT_ARRAY_INDEXOF = "In list";
Blockly.Msg.BIT_ARRAY_INDEXOF_FIND = "find";
Blockly.Msg.BIT_ARRAY_INDEXOF_FIRST = "first";
Blockly.Msg.BIT_ARRAY_INDEXOF_LAST = "last";
Blockly.Msg.BIT_ARRAY_INDEXOF_NUM = "occurrence of item";
Blockly.Msg.BIT_ARRAY_INDEXOF_POSITION = "";
Blockly.Msg.BIT_ARRAY_INDEXOF_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array06";
Blockly.Msg.BIT_ARRAY_SORT = "In list";
Blockly.Msg.BIT_ARRAY_SORT_DEPEND = "sort";
Blockly.Msg.BIT_ARRAY_SORT_NUMERIC = "numeric";
Blockly.Msg.BIT_ARRAY_SORT_TEXT = "alphabetic";
Blockly.Msg.BIT_ARRAY_SORT_IGNORE_CASE = "alphabetic, ignore case";
Blockly.Msg.BIT_ARRAY_SORT_DO = "";
Blockly.Msg.BIT_ARRAY_SORT_TYPE1 = "ascending";
Blockly.Msg.BIT_ARRAY_SORT_TYPE2 = "descending";
Blockly.Msg.BIT_ARRAY_SORT_TYPE = "";
Blockly.Msg.BIT_ARRAY_SORT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array07";
Blockly.Msg.BIT_ARRAY_SPLIT = "In list";
Blockly.Msg.BIT_ARRAY_SPLIT_USE = "with delimiter";
Blockly.Msg.BIT_ARRAY_SPLIT_SPLIT = "list from text";
Blockly.Msg.BIT_ARRAY_SPLIT_JOIN = "text from list";
Blockly.Msg.BIT_ARRAY_SPLIT_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array08";
Blockly.Msg.BIT_ARRAY_LENGTH = "Length of";
Blockly.Msg.BIT_ARRAY_LENGTH_LENGTH = "";
Blockly.Msg.BIT_ARRAY_LENGTH_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/array.html#array09";

// Web:Bit - Color

Blockly.Msg.COLOUR_BLEND_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/color.html#color04";
Blockly.Msg.COLOUR_PICKER_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/color.html#color01";
Blockly.Msg.COLOUR_RANDOM_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/color.html#color02";
Blockly.Msg.COLOUR_RGB_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/color.html#color03";

// Web:Bit - Function

Blockly.Msg.PROCEDURES_CALLNORETURN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/function.html#function01";
Blockly.Msg.PROCEDURES_CALLRETURN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/function.html#function02";
Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/function.html#function01";
Blockly.Msg.PROCEDURES_DEFRETURN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/function.html#function02";
Blockly.Msg.PROCEDURES_IFRETURN_HELPURL = Blockly.Msg.TUTORIAL_URL + "basic/function.html#function03";

// Web:Bit - Monster

Blockly.Msg.MONSTER_GREEN = "GREEN";
Blockly.Msg.MONSTER_RED = "RED";
Blockly.Msg.MONSTER_YELLOW = "YELLOW";
Blockly.Msg.MONSTER_BLUE = "BLUE";
Blockly.Msg.MONSTER_ALL = "All";
Blockly.Msg.MONSTER_TALK = "talks";
Blockly.Msg.MONSTER_DONT_TALK = "quiet";
Blockly.Msg.MONSTER_TALK_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster01";
Blockly.Msg.MONSTER_SHOWIMG = "show image";
Blockly.Msg.MONSTER_SHOWIMG_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster02";
Blockly.Msg.MONSTER_EMOTION = "emotion";
Blockly.Msg.MONSTER_EMOTION_NORMAL = "notmal";
Blockly.Msg.MONSTER_EMOTION_HAPPY = "happy";
Blockly.Msg.MONSTER_EMOTION_SUPRISED = "suprised";
Blockly.Msg.MONSTER_EMOTION_SAD = "sad";
Blockly.Msg.MONSTER_EMOTION_ANGRY = "angry";
Blockly.Msg.MONSTER_EMOTION_RANDOM = "random";
Blockly.Msg.MONSTER_EMOTION_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster03";
Blockly.Msg.MONSTER_MOVE_TO = "move";
Blockly.Msg.MONSTER_MOVE_TOP = "up";
Blockly.Msg.MONSTER_MOVE_BOTTOM = "down";
Blockly.Msg.MONSTER_MOVE_LEFT = "left";
Blockly.Msg.MONSTER_MOVE_RIGHT = "right";
Blockly.Msg.MONSTER_MOVE_RANDOM = "random";
Blockly.Msg.MONSTER_MOVE_MOUSE = "mouse-pointer";
Blockly.Msg.MONSTER_MOVE = "";
Blockly.Msg.MONSTER_MOVE_PIXEL = "pixel";
Blockly.Msg.MONSTER_MOVE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster04";
Blockly.Msg.MONSTER_MOVETO_X = "move to x";
Blockly.Msg.MONSTER_MOVETO_Y = "y";
Blockly.Msg.MONSTER_MOVETO_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster05";
Blockly.Msg.MONSTER_ROTATE_LEFT = "turn left";
Blockly.Msg.MONSTER_ROTATE_RIGHT = "turn right";
Blockly.Msg.MONSTER_ROTATE_ANGLE = "degrees";
Blockly.Msg.MONSTER_ROTATE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster06";
Blockly.Msg.MONSTER_FACETO = "face to";
Blockly.Msg.MONSTER_FACETO_ANGLE = "degrees";
Blockly.Msg.MONSTER_FACETO_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster07";
Blockly.Msg.MONSTER_FACE_MOUSE_AUTO = "auto";
Blockly.Msg.MONSTER_FACE_MOUSE_STOP = "stop";
Blockly.Msg.MONSTER_FACE_MOUSE = "face to mouse-pointer";
Blockly.Msg.MONSTER_FACE_MOUSE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster08";
Blockly.Msg.MONSTER_STATE_S = "'s ";
Blockly.Msg.MONSTER_STATE_X = "x position";
Blockly.Msg.MONSTER_STATE_Y = "y position";
Blockly.Msg.MONSTER_STATE_W = "width";
Blockly.Msg.MONSTER_STATE_H = "height";
Blockly.Msg.MONSTER_STATE_ANGLE = "rotate degree";
Blockly.Msg.MONSTER_STATE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster09";
Blockly.Msg.MONSTER_SIZE = "change size";
Blockly.Msg.MONSTER_SIZE_BIG = "big by";
Blockly.Msg.MONSTER_SIZE_SMALL = "small by";
Blockly.Msg.MONSTER_SIZE_PIXEL = "pixel";
Blockly.Msg.MONSTER_SIZE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster10";
Blockly.Msg.MONSTER_SIZETO = "set size to";
Blockly.Msg.MONSTER_SIZETO_PERCENT = "%";
Blockly.Msg.MONSTER_SIZETO_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster11";
Blockly.Msg.MONSTER_DISPLAY = "display";
Blockly.Msg.MONSTER_DISPLAY_HIDE = "show";
Blockly.Msg.MONSTER_DISPLAY_SHOW = "hide";
Blockly.Msg.MONSTER_DISPLAY_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster12";
Blockly.Msg.MONSTER_ZINDEX = "go to";
Blockly.Msg.MONSTER_ZINDEX_TOP = "front layer";
Blockly.Msg.MONSTER_ZINDEX_BOTTOM = "back layer";
Blockly.Msg.MONSTER_ZINDEX_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster14";
Blockly.Msg.MONSTER_RESET = "reset";
Blockly.Msg.MONSTER_RESET_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/basic.html#monster13";
Blockly.Msg.MONSTER_CLICK = "When mouse clicked";
Blockly.Msg.MONSTER_CLICK_DO = "Do";
Blockly.Msg.MONSTER_CLICK_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event01";
Blockly.Msg.MONSTER_HOVER = "When mouse-pointer touched";
Blockly.Msg.MONSTER_HOVER_ENTER = "Enter:";
Blockly.Msg.MONSTER_HOVER_LEAVE = "Leave:";
Blockly.Msg.MONSTER_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event02";
Blockly.Msg.MONSTER_COLLISION_WHEN = "When";
Blockly.Msg.MONSTER_COLLISION = "touched";
Blockly.Msg.MONSTER_COLLISION_DO = "Do";
Blockly.Msg.MONSTER_COLLISION_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event03";
Blockly.Msg.MONSTER_STAGE_WHEN = "When";
Blockly.Msg.MONSTER_STAGE = "touched the stage's";
Blockly.Msg.MONSTER_STAGE_EDGE = "4 edge";
Blockly.Msg.MONSTER_STAGE_TOP_BOTTOM = "top or bottom edge";
Blockly.Msg.MONSTER_STAGE_LEFT_RIGHT = "left or right edge";
Blockly.Msg.MONSTER_STAGE_TOP = "top edge";
Blockly.Msg.MONSTER_STAGE_BOTTOM = "bottom edge";
Blockly.Msg.MONSTER_STAGE_LEFT = "left edge";
Blockly.Msg.MONSTER_STAGE_RIGHT = "right edge";
Blockly.Msg.MONSTER_STAGE_DO = "Do";
Blockly.Msg.MONSTER_STAGE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event04";
Blockly.Msg.MONSTER_STAGE_REBOUND = "If on edge, bounce";
Blockly.Msg.MONSTER_STAGE_REBOUND_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event05";
Blockly.Msg.MONSTER_BACKGROUND_COLOR = "Change the background color of the stage to";
Blockly.Msg.MONSTER_BACKGROUND_COLOR_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event06";
Blockly.Msg.MONSTER_BACKGROUND_IMG = "Change the background image of the stage to";
Blockly.Msg.MONSTER_BACKGROUND_IMG_HELPURL = Blockly.Msg.MONSTER_BACKGROUND_COLOR_HELPURL;
Blockly.Msg.MONSTER_STAGE_MAX = "Set the stage to full screen";
Blockly.Msg.MONSTER_STAGE_MAX_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event07";
Blockly.Msg.MONSTER_STAGE_SIZE = "Stage's";
Blockly.Msg.MONSTER_STAGE_SIZE_WIDTH = "width";
Blockly.Msg.MONSTER_STAGE_SIZE_HEIGHT = "height";
Blockly.Msg.MONSTER_STAGE_SIZE_HELPURL = Blockly.Msg.TUTORIAL_URL + "monster/event.html#event08";
Blockly.Msg.MONSTER_MOUSE = "Mouse-pointer's";
Blockly.Msg.MONSTER_MOUSE_COORDINATE = "position";

// Web:Bit - Detect

Blockly.Msg.BIT_KEYCODE = "When";
Blockly.Msg.BIT_KEYCODE_KEYDOWN = "key pressed";
Blockly.Msg.BIT_KEYCODE_KEYUP = "key up";
Blockly.Msg.BIT_KEYCODE_SPACE = "space";
Blockly.Msg.BIT_KEYCODE_UP = "up ▲";
Blockly.Msg.BIT_KEYCODE_DOWN = "down ▼";
Blockly.Msg.BIT_KEYCODE_LEFT = "left ◀";
Blockly.Msg.BIT_KEYCODE_RIGHT = "right ▶";
Blockly.Msg.BIT_KEYCODE_DO = "Do";
Blockly.Msg.BIT_KEYCODE_HELPURL = Blockly.Msg.TUTORIAL_URL + "detect/keyboard.html";
Blockly.Msg.BIT_TIME_HELPURL = Blockly.Msg.TUTORIAL_URL + "detect/time.html";
Blockly.Msg.BIT_DATE_HELPURL = Blockly.Msg.BIT_TIME_HELPURL;
Blockly.Msg.BIT_INPUT = "Enter text";
Blockly.Msg.BIT_INPUT_COMMENT = "The program will not continue until the text is entered";
Blockly.Msg.BIT_INPUT_VAL = "text";
Blockly.Msg.BIT_INPUT_HELPURL= Blockly.Msg.TUTORIAL_URL + "detect/input.html";

// Web:Bit - Sound

Blockly.Msg.SOUND_01_CAT = "Cat";
Blockly.Msg.SOUND_01_DOG = "Dog";
Blockly.Msg.SOUND_01_LION = "Lion";
Blockly.Msg.SOUND_01_GOAT = "Goat";
Blockly.Msg.SOUND_01_ELEPHANT = "Elephant";
Blockly.Msg.SOUND_01_CHICKEN = "Chicken";
Blockly.Msg.SOUND_01_CHICK = "Chick";
Blockly.Msg.SOUND_01_DUCK = "duck";
Blockly.Msg.SOUND_01_CROW = "crow";
Blockly.Msg.SOUND_01_MONKEY = "monkey";
Blockly.Msg.SOUND_01_FROG = "frog";
Blockly.Msg.SOUND_01_MOUSE = "mouse";
Blockly.Msg.SOUND_01_PIG = "pig";
Blockly.Msg.SOUND_01_RANDOM = "random";
Blockly.Msg.SOUND_01 = "Animal's sound";

Blockly.Msg.SOUND_02_01 = "right";
Blockly.Msg.SOUND_02_02 = "don";
Blockly.Msg.SOUND_02_03 = "beep";
Blockly.Msg.SOUND_02_04 = "du";
Blockly.Msg.SOUND_02_05 = "hit";
Blockly.Msg.SOUND_02_06 = "qiang";
Blockly.Msg.SOUND_02_07 = "laser";
Blockly.Msg.SOUND_02_08 = "ding dong";
Blockly.Msg.SOUND_02_09 = "dice";
Blockly.Msg.SOUND_02_10 = "bubble";
Blockly.Msg.SOUND_02_11 = "horn";
Blockly.Msg.SOUND_02_12 = "gold";
Blockly.Msg.SOUND_02_13 = "jump";
Blockly.Msg.SOUND_02_14 = "dead";
Blockly.Msg.SOUND_02_15 = "bell";
Blockly.Msg.SOUND_02_RANDOM = Blockly.Msg.SOUND_01_RANDOM;
Blockly.Msg.SOUND_02 = "Special sound";

Blockly.Msg.SOUND_03_01 = "sneeze";
Blockly.Msg.SOUND_03_02 = "laugh";
Blockly.Msg.SOUND_03_03 = "cough";
Blockly.Msg.SOUND_03_04 = "kiss";
Blockly.Msg.SOUND_03_05 = "applaud";
Blockly.Msg.SOUND_03_06 = "cry";
Blockly.Msg.SOUND_03_07 = "burp";
Blockly.Msg.SOUND_03_08 = "fart";
Blockly.Msg.SOUND_03_09 = "whistle";
Blockly.Msg.SOUND_03_10 = "snore";
Blockly.Msg.SOUND_03_11 = "sigh";
Blockly.Msg.SOUND_03_12 = "startled";
Blockly.Msg.SOUND_03_RANDOM = Blockly.Msg.SOUND_01_RANDOM;
Blockly.Msg.SOUND_03 = "Human's sound";

Blockly.Msg.SOUND_HELPURL = Blockly.Msg.TUTORIAL_URL + "sound/sound-effect.html";

Blockly.Msg.BIT_SPEAK_TEXT = "Speak";
Blockly.Msg.BIT_SPEAK_SETTING = "Setting";
Blockly.Msg.BIT_SPEAK_TOOLTIPS = "After speaking aloud, the following program will continue";
Blockly.Msg.BIT_SPEAK_HELPURL = Blockly.Msg.TUTORIAL_URL + "sound/speak-aloud.html";
Blockly.Msg.BIT_SPEAK_SETTING_LANG = "language";
Blockly.Msg.BIT_SPEAK_SETTING_ZHTW = "Chinese";
Blockly.Msg.BIT_SPEAK_SETTING_EN = "English";
Blockly.Msg.BIT_SPEAK_SETTING_JP = "Japanese";
Blockly.Msg.BIT_SPEAK_SETTING_T = " tone";
Blockly.Msg.BIT_SPEAK_SETTING_T2 = "very height";
Blockly.Msg.BIT_SPEAK_SETTING_T15 = "height";
Blockly.Msg.BIT_SPEAK_SETTING_T1 = "normal";
Blockly.Msg.BIT_SPEAK_SETTING_T05 = "low";
Blockly.Msg.BIT_SPEAK_SETTING_T01 = "very low";
Blockly.Msg.BIT_SPEAK_SETTING_S = " speed";
Blockly.Msg.BIT_SPEAK_SETTING_S2 = "very fast";
Blockly.Msg.BIT_SPEAK_SETTING_S15 = "fast";
Blockly.Msg.BIT_SPEAK_SETTING_S1 = "normal";
Blockly.Msg.BIT_SPEAK_SETTING_S07 = "slow";
Blockly.Msg.BIT_SPEAK_SETTING_S05 = "very slow";
Blockly.Msg.BIT_SPEAK_SETTING_HELPURL = Blockly.Msg.BIT_SPEAK_HELPURL;

Blockly.Msg.SPEECH = "Speech recognition, language";
Blockly.Msg.SPEECH_TW = "Chinese";
Blockly.Msg.SPEECH_EN = "English";
Blockly.Msg.SPEECH_SUPPORT = " (Chrome、Android only )";
Blockly.Msg.SPEECH_COMMENT = "The following program will continue after speech recognition";
Blockly.Msg.SPEECH_VAL = "Recognized text";
Blockly.Msg.SPEECH_HELPURL = Blockly.Msg.TUTORIAL_URL + "sound/speech-recognition.html";

// Web:Bit - Board

Blockly.Msg.BIT_BOARD_USE = "USe";
Blockly.Msg.BIT_BOARD_DO = "Do";
Blockly.Msg.BIT_BOARD_BIT = "Simulator";
Blockly.Msg.BIT_BOARD_USB = "USB";
Blockly.Msg.BIT_BOARD_WIFI = "Wi-Fi";
Blockly.Msg.BIT_BOARD_CONTROL = "control";
Blockly.Msg.BIT_BOARD_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/board.html";

// Web:Bit - I/O Pin

Blockly.Msg.WEBDUINO_PIN_READ = "Read Value from";
Blockly.Msg.WEBDUINO_PIN_ANALOG_WRITE = "Analog Output to Pin";
Blockly.Msg.WEBDUINO_PIN_DIGITAL_WRITE = "Digital Output to Pin";
Blockly.Msg.WEBDUINO_PIN_VALUE = "";
Blockly.Msg.WEBDUINO_PIN_DIN = "Digital Input";
Blockly.Msg.WEBDUINO_PIN_AIN = "Analog Input";
Blockly.Msg.WEBDUINO_PIN_READ_DIGITAL_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/io-pin.html#pin01";
Blockly.Msg.WEBDUINO_PIN_READ_ANALOG_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/io-pin.html#pin02";
Blockly.Msg.WEBDUINO_PIN_WRITE_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/io-pin.html#pin03";

// Web:Bit - Matrix

Blockly.Msg.BIT_MATRIX_COLOR = "Matrix LED:";
Blockly.Msg.BIT_MATRIX_COLOR_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix01";
Blockly.Msg.BIT_MATRIX_COLOR_ARRAY = "draw image";
Blockly.Msg.BIT_MATRIX_COLOR_ARRAY_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix02";
Blockly.Msg.BIT_MATRIX_EMOJI_HAPPY = "happy";
Blockly.Msg.BIT_MATRIX_EMOJI_CRY = "cry";
Blockly.Msg.BIT_MATRIX_EMOJI_01 = "scissors";
Blockly.Msg.BIT_MATRIX_EMOJI_02 = "rock";
Blockly.Msg.BIT_MATRIX_EMOJI_03 = "paper";
Blockly.Msg.BIT_MATRIX_EMOJI_RANDOM = "random";
Blockly.Msg.BIT_MATRIX_EMOJI = "emoji";
Blockly.Msg.BIT_MATRIX_EMOJI_COLOR = "color";
Blockly.Msg.BIT_MATRIX_EMOJI_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix03";
Blockly.Msg.BIT_MATRIX_CHARACTER = "Matrix LED displayed one word";
Blockly.Msg.BIT_MATRIX_CHARACTER_COLOR = "color";
Blockly.Msg.BIT_MATRIX_CHARACTER_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix04";
Blockly.Msg.BIT_MATRIX_STRING = "Matrix LED Marquee";
Blockly.Msg.BIT_MATRIX_STRING_COLOR = "color";
Blockly.Msg.BIT_MATRIX_STRING_PLAY = "play";
Blockly.Msg.BIT_MATRIX_STRING_ONCE = "once";
Blockly.Msg.BIT_MATRIX_STRING_UNLIMITED = "unlimit";
Blockly.Msg.BIT_MATRIX_STRING_SPEED = ' speed';
Blockly.Msg.BIT_MATRIX_STRING_FAST = "fast";
Blockly.Msg.BIT_MATRIX_STRING_NORMAL = "normal";
Blockly.Msg.BIT_MATRIX_STRING_SLOW = "slowly";
Blockly.Msg.BIT_MATRIX_STRING_COMMENT = "If you select once, the following program will continue to run after the marquee ends."
Blockly.Msg.BIT_MATRIX_STRING_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix05";
Blockly.Msg.BIT_MATRIX_SINGLE = "Matrix LED:";
Blockly.Msg.BIT_MATRIX_SINGLE_NUM = "light's color";
Blockly.Msg.BIT_MATRIX_SINGLE_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix07";
Blockly.Msg.BIT_MATRIX_XY = "Matrix LED:";
Blockly.Msg.BIT_MATRIX_XY_X = "x";
Blockly.Msg.BIT_MATRIX_XY_Y = "y";
Blockly.Msg.BIT_MATRIX_XY_COLOR = "color";
Blockly.Msg.BIT_MATRIX_XY_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix08";
Blockly.Msg.BIT_MATRIX_BRIGHTNESS = "Set brightness (0~20)";
Blockly.Msg.BIT_MATRIX_BRIGHTNESS_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix09";
Blockly.Msg.BIT_MATRIX_OFF = "Turn the Matrix LED off";
Blockly.Msg.BIT_MATRIX_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/rgbled-matrix.html#rgbledmatrix10";

// Web:Bit - Button

Blockly.Msg.BIT_AB_BUTTON = "When button";
Blockly.Msg.BIT_AB_BUTTON_A = "A";
Blockly.Msg.BIT_AB_BUTTON_B = "B";
Blockly.Msg.BIT_AB_BUTTON_AB = "A+B";
Blockly.Msg.BIT_AB_BUTTON_S = "";
Blockly.Msg.BIT_AB_BUTTON_PRESSED = "pressed";
Blockly.Msg.BIT_AB_BUTTON_RELEASE = "release";
Blockly.Msg.BIT_AB_BUTTON_LONGPRESS = "long press";
Blockly.Msg.BIT_AB_BUTTON_DO = "Do";
Blockly.Msg.BIT_AB_BUTTON_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/ab-button.html";

// Web:Bit - Detected

Blockly.Msg.BIT_DETECTED = "偵測光線＆溫度 ( 重複無限次 )";
Blockly.Msg.BIT_DETECTED_DO = "執行";
Blockly.Msg.BIT_DETECTED_PHOTOCELL = "brightness";
Blockly.Msg.BIT_DETECTED_PHOTOCELL_LEFT = "left";
Blockly.Msg.BIT_DETECTED_PHOTOCELL_RIGHT = "right";
Blockly.Msg.BIT_DETECTED_PHOTOCELL_VAL = "( lumen )";
Blockly.Msg.BIT_DETECTED_PHOTOCELL_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/photocell-thermistor.html#pt01";
Blockly.Msg.BIT_DETECTED_TEMP = "temperature ( Celsius )";
Blockly.Msg.BIT_DETECTED_TEMP_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/photocell-thermistor.html#pt02";

// Web:Bit - Buzzer

Blockly.Msg.BIT_BUZZER_4 = "low";
Blockly.Msg.BIT_BUZZER_5 = "middle";
Blockly.Msg.BIT_BUZZER_6 = "height";
Blockly.Msg.BIT_BUZZER_PLAY = "Play tone";
Blockly.Msg.BIT_BUZZER_PLAY_CONTINUE = "continue";
Blockly.Msg.BIT_BUZZER_TEMPO = "meter";
Blockly.Msg.BIT_BUZZER_MUTE = "mute";
Blockly.Msg.BIT_BUZZER_PLAY_MUTE = "Play rest";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC = "Play music";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_M1 = "Super Mario";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_M2 = "Super Mario 2";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_M3 = "Do Re Mi";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_M4 = "brother and father";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_M5 = "Jingle Bells";
Blockly.Msg.BIT_BUZZER_PLAY_EVENT_STOP = 'stop';
Blockly.Msg.BIT_BUZZER_PLAY_EVENT_PAUSE = 'pause';
Blockly.Msg.BIT_BUZZER_PLAY_EVENT_PLAY = 'continue';
Blockly.Msg.BIT_BUZZER_PLAY_EVENT = 'play';
Blockly.Msg.BIT_BUZZER_PLAY_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/buzzer.html#buzzer01";
Blockly.Msg.BIT_BUZZER_PLAY_MUTE_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/buzzer.html#buzzer02";
Blockly.Msg.BIT_BUZZER_PLAY_MUSIC_HELPURL =  Blockly.Msg.TUTORIAL_URL + "board/buzzer.html#buzzer03";
Blockly.Msg.BIT_BUZZER_PLAY_EVENT_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/buzzer.html#buzzer04";

// Web:Bit - MPU9250

Blockly.Msg.BIT_MPU9250_ICON = "bit-mpu9250-icon en";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_SHAKE = "shake";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_UP = "face up";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_ROLLBACK = "roll back";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_ROLLRIGHT = "roll right";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_TURNRIGHT = "turn right";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_PEACE = "peace";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_BACK = "back up";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_ROLLFRONT = "roll front";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_ROLLLEFT = "roll left";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_TRUNLEFT = "turn left";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_NORTH = "face north";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_WEST = "face west";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_SOUTH = "face south";
Blockly.Msg.BIT_MPU9250_TYPE_TITLE_EAST = "face east";
Blockly.Msg.BIT_MPU9250_IF = "If board";
Blockly.Msg.BIT_MPU9250_DO = "Do";
Blockly.Msg.BIT_MPU9250_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/mpu9250.html#mpu01";
Blockly.Msg.BIT_MPU9250_COMMENT = "Stand-alone use";
Blockly.Msg.BIT_MPU9250_VAL_ROLL = "roll ( ↕ )";
Blockly.Msg.BIT_MPU9250_VAL_PITCH = "pitch ( ↔ )";
Blockly.Msg.BIT_MPU9250_VAL_YAW = "yaw ( ↻ )";
Blockly.Msg.BIT_MPU9250_VAL_AZI = "azi ( ➹ )";
Blockly.Msg.BIT_MPU9250_VAL_ACCX = "x ( acc )";
Blockly.Msg.BIT_MPU9250_VAL_ACCY = "y ( acc )";
Blockly.Msg.BIT_MPU9250_VAL_ACCZ = "z ( acc )";
Blockly.Msg.BIT_MPU9250_VAL_GYRX = "x ( gyr )";
Blockly.Msg.BIT_MPU9250_VAL_GYRY = "y ( gyr )";
Blockly.Msg.BIT_MPU9250_VAL_GYRZ = "z ( gyr )";
Blockly.Msg.BIT_MPU9250_VAL_MAGX = "x ( mag )";
Blockly.Msg.BIT_MPU9250_VAL_MAGY = "y ( mag )";
Blockly.Msg.BIT_MPU9250_VAL_MAGZ = "z ( mag )";
Blockly.Msg.BIT_MPU9250_VAL = "";
Blockly.Msg.BIT_MPU9250_VAL_HELPURL = Blockly.Msg.TUTORIAL_URL + "board/mpu9250.html#mpu02";




